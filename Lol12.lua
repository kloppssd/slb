...
-- Create tabs
local Tabs = {
    Main = Window:AddTab({ Title = "ğŸ›¡ï¸ Main", Icon = "shield" }),
    ...
    Server = Window:AddTab({ Title = "ğŸŒ Server", Icon = "globe" })
}

-- MAIN TAB
Tabs.Main:AddToggle("SpeedBoost", {
    Title = "âš¡ Speed Boost 8x (Enhanced)",
...
Tabs.Main:AddToggle("AntiTrap", {
    Title = "ğŸš« Anti Trap (Movement Freedom)",
...
Tabs.Main:AddToggle("AntiStun", {
    Title = "ğŸ›¡ï¸ Anti Stun (Enhanced with Attacker Detection)",
...
Tabs.Main:AddToggle("InfiniteJump", {
    Title = "ğŸ¦˜ Infinite Jump (Enhanced)",
...
Tabs.Visual:AddToggle("ESPPlayers", {
    Title = "ğŸ‘¥ ESP Player Highlights",
...
-- MISC TAB
Tabs.Misc:AddSection("ğŸš€ Stealing Tools")
...
Tabs.Misc:AddSection("ğŸ“ Base System")
...
Tabs.Misc:AddButton({
    ...
    Callback = function()
        ...
        statusLabel.Text = basePosition and "âœ… Base Position Set" or "âŒ Base Position Not Set"
        ...
        coordsLabel.Text = basePosition and string.format("ğŸ“ X:%.1f Y:%.1f Z:%.1f", basePosition.X, basePosition.Y, basePosition.Z) or "ğŸ“ No coordinates set"
        ...
        local function updateDisplay()
            if basePosition then
                statusLabel.Text = "âœ… Base Position Set"
                statusLabel.TextColor3 = Color3.fromRGB(100, 255, 100)
                coordsLabel.Text = string.format("ğŸ“ X:%.1f Y:%.1f Z:%.1f", basePosition.X, basePosition.Y, basePosition.Z)
                coordsLabel.TextColor3 = Color3.fromRGB(200, 200, 200)
            else
                statusLabel.Text = "âŒ Base Position Not Set"
                statusLabel.TextColor3 = Color3.fromRGB(255, 100, 100)
                coordsLabel.Text = "ğŸ“ No coordinates set"
...
local moneyLabel = Tabs.Money:AddParagraph({
    Title = "ğŸ’° Current Money",
    Content = "Loading..."
})

Tabs.Money:AddToggle("AutoCollectMoney", {
    Title = "ğŸ”„ Auto Collect Money",
...
-- SERVER TAB
Tabs.Server:AddSection("ğŸ”„ Server Controls")
...
local function updateMoneyLabel()
    ...
    if leaderstats then
        ...
        if money then
            moneyLabel:SetDesc("ğŸ’° " .. tostring(money.Value))
        end
    else
        moneyLabel:SetDesc("ğŸ’° 0 (No leaderstats found)")
    end
end
...
RunService.Heartbeat:Connect(function()
    ...
    if tick() % 2 < 0.016 then
        playerCountLabel:SetDesc("ğŸ‘¥ " .. #Players:GetPlayers() .. "/" .. Players.MaxPlayers)
        
        local ping = player:GetNetworkPing()
        if ping then
            pingLabel:SetDesc("ğŸ“¡ " .. math.floor(ping * 1000) .. "ms")
        end
    end
end)
