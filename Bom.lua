...
local script = G2L["f"];
	...
	-- ĞĞ±Ğ½Ğ¾Ğ²Ğ»ĞµĞ½Ğ¸Ğµ Ğ²Ğ¸Ğ´Ğ° ĞºĞ½Ğ¾Ğ¿ĞºĞ¸
	...
	-- Ğ¤ÑƒĞ½ĞºÑ†Ğ¸Ñ Ğ²Ğ¾Ğ·Ğ´ÑƒÑˆĞ½Ğ¾Ğ³Ğ¾ Ğ¿Ñ€Ñ‹Ğ¶ĞºĞ°
	local function safeAirJump()
		...
		-- ĞĞ¿Ñ‚Ğ¸Ğ¼Ğ¸Ğ·Ğ°Ñ†Ğ¸Ñ ÑĞµÑ‚ĞµĞ²Ğ¾Ğ³Ğ¾ Ğ²Ğ»Ğ°Ğ´ĞµĞ½Ğ¸Ñ
		if rootPart:CanSetNetworkOwnership() then
			rootPart:SetNetworkOwner(player)
		end
	
		-- Ğ¡Ğ¾Ğ·Ğ´Ğ°Ğ½Ğ¸Ğµ Ğ²Ñ€ĞµĞ¼ĞµĞ½Ğ½Ğ¾Ğ³Ğ¾ BodyVelocity
		local bodyVelocity = Instance.new("BodyVelocity")
		bodyVelocity.MaxForce = Vector3.new(0, math.huge, 0)
		bodyVelocity.Velocity = Vector3.new(0, JUMP_FORCE, 0)
		bodyVelocity.Parent = rootPart
	
		-- ĞÑ‡Ğ¸ÑÑ‚ĞºĞ° Ñ‡ĞµÑ€ĞµĞ· ÑĞ¾ĞµĞ´Ğ¸Ğ½ĞµĞ½Ğ¸Ğµ (Ğ±Ğ¾Ğ»ĞµĞµ Ğ½Ğ°Ğ´ĞµĞ¶Ğ½Ğ¾, Ñ‡ĞµĞ¼ task.wait)
		local connection
	...
	-- ĞĞ±Ñ€Ğ°Ğ±Ğ¾Ñ‚Ñ‡Ğ¸Ğº Ğ¿Ñ€Ñ‹Ğ¶ĞºĞ°
	...
	-- ĞĞ°Ñ‡Ğ°Ğ»ÑŒĞ½Ğ¾Ğµ ÑĞ¾ÑÑ‚Ğ¾ÑĞ½Ğ¸Ğµ ĞºĞ½Ğ¾Ğ¿ĞºĞ¸
...
local script = G2L["12"];
	...
	local function highJump()
		...
		-- ĞÑ‡Ğ¸ÑÑ‚ĞºĞ° Ğ¿Ñ€ĞµĞ´Ñ‹Ğ´ÑƒÑ‰Ğ¸Ñ… Ğ¾Ğ±ÑŠĞµĞºÑ‚Ğ¾Ğ²
		cleanJump()
	
		-- Ğ£ÑÑ‚Ğ°Ğ½Ğ¾Ğ²ĞºĞ° ÑĞµÑ‚ĞµĞ²Ğ¾Ğ³Ğ¾ Ğ²Ğ»Ğ°Ğ´ĞµĞ½Ğ¸Ñ
		if rootPart:CanSetNetworkOwnership() then
			rootPart:SetNetworkOwner(player)
		end
	
		-- Ğ¡Ğ¾Ğ·Ğ´Ğ°Ğ½Ğ¸Ğµ Ñ„Ğ¸Ğ·Ğ¸ĞºĞ¸ Ğ¿Ñ€Ñ‹Ğ¶ĞºĞ°
		...
		-- Ğ¢Ğ°Ğ¹Ğ¼ĞµÑ€ Ğ¾Ñ‚ĞºĞ»ÑÑ‡ĞµĞ½Ğ¸Ñ Ğ¿Ñ€Ñ‹Ğ¶ĞºĞ°
		activeConnection = runService.Heartbeat:Connect(function()
			if os.clock() - lastJumpTime >= JUMP_DURATION then
				cleanJump()
	
				-- Ğ¡Ğ±Ñ€Ğ¾Ñ ÑĞµÑ‚ĞµĞ²Ğ¾Ğ³Ğ¾ Ğ²Ğ»Ğ°Ğ´ĞµĞ½Ğ¸Ñ
	...
	button.MouseButton1Click:Connect(function()
		...
		-- ĞŸÑ€Ğ¸Ğ½ÑƒĞ´Ğ¸Ñ‚ĞµĞ»ÑŒĞ½Ğ°Ñ Ğ¾Ñ‡Ğ¸ÑÑ‚ĞºĞ° Ğ¿Ñ€Ğ¸ Ğ²Ñ‹ĞºĞ»ÑÑ‡ĞµĞ½Ğ¸Ğ¸
		if not isOn then
			cleanJump()
		end
	end)
	...
	-- ĞÑ‡Ğ¸ÑÑ‚ĞºĞ° Ğ¿Ñ€Ğ¸ ÑĞ¼ĞµÑ€Ñ‚Ğ¸ Ğ¿ĞµÑ€ÑĞ¾Ğ½Ğ°Ğ¶Ğ°
	player.CharacterAdded:Connect(function(character)
		character:WaitForChild("Humanoid").Died:Connect(cleanJump)
	end)
	
	-- Ğ˜Ğ½Ğ¸Ñ†Ğ¸Ğ°Ğ»Ğ¸Ğ·Ğ°Ñ†Ğ¸Ñ ĞºĞ½Ğ¾Ğ¿ĞºĞ¸
	button.BackgroundColor3 = Color3.fromRGB(167, 31, 31)
end;
task.spawn(C_12);
-- StarterGui.s.main.main.speed.LocalScript
...
local script = G2L["15"];
	...
	if floatMenu then
		...
		-- ĞŸĞ¾Ğ´ĞºĞ»ÑÑ‡ĞµĞ½Ğ¸Ğµ ÑĞ¾Ğ±Ñ‹Ñ‚Ğ¸Ğ¹
		...
		-- ĞÑ‡Ğ¸ÑÑ‚ĞºĞ° Ğ¿Ñ€Ğ¸ ÑƒĞ½Ğ¸Ñ‡Ñ‚Ğ¾Ğ¶ĞµĞ½Ğ¸Ğ¸
		...
		-- Ğ˜Ğ½Ğ¸Ñ†Ğ¸Ğ°Ğ»Ğ¸Ğ·Ğ°Ñ†Ğ¸Ñ Ğ½Ğ°Ñ‡Ğ°Ğ»ÑŒĞ½Ğ¾Ğ³Ğ¾ ÑĞ¾ÑÑ‚Ğ¾ÑĞ½Ğ¸Ñ
		updateButtonState()
	else
...
local script = G2L["1b"];
	...
	-- ĞĞ°ÑÑ‚Ñ€Ğ¾Ğ¹ĞºĞ¸ UIListLayout
	uiListLayout.SortOrder = Enum.SortOrder.LayoutOrder
	uiListLayout.FillDirection = Enum.FillDirection.Vertical
	uiListLayout.Padding = UDim.new(0, 5)
	uiListLayout.HorizontalAlignment = Enum.HorizontalAlignment.Center
	
	-- Ğ¤ÑƒĞ½ĞºÑ†Ğ¸Ñ Ğ´Ğ»Ñ Ğ¾Ğ±Ğ½Ğ¾Ğ²Ğ»ĞµĞ½Ğ¸Ñ CanvasSize
	...
	-- Ğ¡Ğ¾Ğ±Ñ‹Ñ‚Ğ¸Ñ Ğ´Ğ»Ñ Ğ¾Ğ±Ğ½Ğ¾Ğ²Ğ»ĞµĞ½Ğ¸Ñ
